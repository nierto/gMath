#!/usr/bin/env python3
"""
Generate FASC pipeline ULP reference data with 1000+ samples per function.
...
FASC pipeline: gmath("1.5").exp() → evaluate → as_binary_storage → ULP compare.

Output: tests/data/fasc_ulp_refs_{q64_64,q128_128,q256_256}.rs
...
def write_unary_refs(f, const_name, points, mp_func, frac_bits, total_bits):
    ...
    The FASC pipeline will parse the string, so the reference must account for
...
def main():
    ...
    unary_funcs = [
        ("FASC_EXP_REFS", exp_pts, exp),
        ("FASC_LN_REFS", ln_pts, log),
        ("FASC_SQRT_REFS", sqrt_pts, sqrt),
        ("FASC_SIN_REFS", sin_pts, sin),
        ("FASC_COS_REFS", cos_pts, cos),
        ("FASC_TAN_REFS", tan_pts, tan),
        ("FASC_ATAN_REFS", atan_pts, atan),
        ("FASC_ASIN_REFS", asin_pts, asin),
        ("FASC_ACOS_REFS", acos_pts, acos),
        ("FASC_SINH_REFS", sinh_pts, sinh),
        ("FASC_COSH_REFS", cosh_pts, cosh),
        ("FASC_TANH_REFS", tanh_pts, tanh),
        ("FASC_ASINH_REFS", asinh_pts, asinh),
        ("FASC_ACOSH_REFS", acosh_pts, acosh),
        ("FASC_ATANH_REFS", atanh_pts, atanh),
    ]

    binary_funcs = [
        ("FASC_ATAN2_REFS", atan2_pts, atan2),
        ("FASC_POW_INTEGER_REFS", pow_int_pts, power),
        ("FASC_POW_FRACTIONAL_REFS", pow_frac_pts, power),
    ]

    grand_total = 0

    for profile_name, frac_bits, total_bits in PROFILES:
        filename = os.path.join(DATA_DIR, f"fasc_ulp_refs_{profile_name}.rs")
        n_words = total_bits // 64
        print(f"\n{'='*60}")
        ...
        with open(filename, "w") as f:
            f.write(f"// AUTO-GENERATED by scripts/generate_fasc_ulp_references.py\n")
            f.write(f"// Q{frac_bits}.{frac_bits} ({total_bits}-bit) FASC pipeline references\n")
            ...
            f.write(f"//\n")
            f.write(f"// Input strings go through gmath() -> full FASC pipeline.\n")
