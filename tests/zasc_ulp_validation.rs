//! ZASC Pipeline ULP Validation — 1000+ samples per transcendental function.
//!
//! Unlike `transcendental_ulp_validation.rs` which calls raw binary functions
//! directly, this test drives through the **full ZASC pipeline**:
//!
...
//!
//! Reference data generated by: scripts/generate_zasc_ulp_references.py
//!
//! Run per profile:
//!   GMATH_PROFILE=embedded   cargo test --test zasc_ulp_validation -- --nocapture
//!   GMATH_PROFILE=balanced   cargo test --test zasc_ulp_validation -- --nocapture
//!   GMATH_PROFILE=scientific cargo test --test zasc_ulp_validation -- --nocapture
...
impl UlpStats {
    ...
    fn report(&self, profile: &str, frac_bits: u32) {
        ...
        eprintln!(
            "ZASC {:8} {}: max={:<12} mean={:<10.1} p99={:<12} points={:<5} guaranteed={}d (worst: {}){}",
    ...
    fn report_throughput(&self, profile: &str, frac_bits: u32, elapsed: std::time::Duration) {
        ...
        eprintln!(
            "ZASC {:8} {}: max={:<4} p99={:<4} {:>5}d | {:>8.0}ns/op  {:>6.1} {} | points={}  (worst: {}){}",
...
macro_rules! zasc_unary_test {
    ($test_name:ident, $func_name:expr, $refs:ident, $max_ulp:expr, $method:ident) => {
        zasc_unary_test!($test_name, $func_name, $refs, $max_ulp, $method, 0);
    };
    ($test_name:ident, $func_name:expr, $refs:ident, $max_ulp:expr, $method:ident, $max_errors:expr) => {
        ...
        fn $test_name() {
            ...
            assert!(
                ...
                "ZASC {} {} max ULP {} exceeds threshold {} (worst: {})",
                $func_name, PROFILE, stats.max_ulp, $max_ulp, stats.worst_label
            );
            assert!(
                stats.errors <= $max_errors,
                "ZASC {} {} had {} evaluation errors out of {} points (max allowed: {})",
...
macro_rules! zasc_binary_test {
    ($test_name:ident, $func_name:expr, $refs:ident, $max_ulp:expr, $method:expr) => {
        ...
        fn $test_name() {
            ...
            assert!(
                stats.errors == 0,
                "ZASC {} {} had {} evaluation errors out of {} points",
...
mod q64_64 {
    ...
    zasc_unary_test!(validate_exp,   "exp",   ZASC_EXP_REFS,   5, exp);
    zasc_unary_test!(validate_ln,    "ln",    ZASC_LN_REFS,    5, ln);
    zasc_unary_test!(validate_sqrt,  "sqrt",  ZASC_SQRT_REFS,  5, sqrt);
    zasc_unary_test!(validate_sin,   "sin",   ZASC_SIN_REFS,   5, sin);
    zasc_unary_test!(validate_cos,   "cos",   ZASC_COS_REFS,   5, cos);
    zasc_unary_test!(validate_atan,  "atan",  ZASC_ATAN_REFS,  5, atan);

    // Composed functions (all 0 ULP):
    zasc_unary_test!(validate_tan,   "tan",   ZASC_TAN_REFS,   5, tan);
    zasc_unary_test!(validate_asin,  "asin",  ZASC_ASIN_REFS,  5, asin);
    zasc_unary_test!(validate_acos,  "acos",  ZASC_ACOS_REFS,  5, acos);

    // Hyperbolic functions (all 0 ULP):
    zasc_unary_test!(validate_sinh,  "sinh",  ZASC_SINH_REFS,  5, sinh);
    zasc_unary_test!(validate_cosh,  "cosh",  ZASC_COSH_REFS,  5, cosh);
    zasc_unary_test!(validate_tanh,  "tanh",  ZASC_TANH_REFS,  5, tanh);
    zasc_unary_test!(validate_asinh, "asinh", ZASC_ASINH_REFS, 5, asinh);
    zasc_unary_test!(validate_acosh, "acosh", ZASC_ACOSH_REFS, 5, acosh);
    zasc_unary_test!(validate_atanh, "atanh", ZASC_ATANH_REFS, 5, atanh);

    // Binary functions (all 0 ULP after to_compute_storage fix):
    zasc_binary_test!(validate_atan2,        "atan2",    ZASC_ATAN2_REFS,             5, |a, b| a.atan2(b));
    zasc_binary_test!(validate_pow_integer,  "pow_int",  ZASC_POW_INTEGER_REFS,       5, |a, b| a.pow(b));
    zasc_binary_test!(validate_pow_frac,     "pow_frac", ZASC_POW_FRACTIONAL_REFS,    5, |a, b| a.pow(b));
}
...
mod q128_128 {
    ...
    zasc_unary_test!(validate_atanh, "atanh", ZASC_ATANH_REFS, 5, atanh, 530);
...
mod q256_256 {
    ...
    zasc_unary_test!(validate_sin,   "sin",   ZASC_SIN_REFS,   5, sin);
    zasc_unary_test!(validate_cos,   "cos",   ZASC_COS_REFS,   5, cos);
    zasc_unary_test!(validate_tan,   "tan",   ZASC_TAN_REFS,   15, tan);
    zasc_unary_test!(validate_atan,  "atan",  ZASC_ATAN_REFS,  5, atan);
    zasc_unary_test!(validate_asin,  "asin",  ZASC_ASIN_REFS,  5, asin);
    zasc_unary_test!(validate_acos,  "acos",  ZASC_ACOS_REFS,  5, acos);
    zasc_unary_test!(validate_sinh,  "sinh",  ZASC_SINH_REFS,  5, sinh);
