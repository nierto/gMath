...
**Author**: Niels Erik Toren — v0.4.24
...
## Entry points — pick by cost, not by correctness

There are two main ways to compute, plus specialised surfaces. They differ in
performance and ergonomics; **the answer they return does not depend on which you
pick.**

| Entry point | What it is | Reach for it when |
| ----------- | ---------- | ----------------- |
| **Canonical / router** — `gmath("…")` → `evaluate` | A lazy expression run through the fractal router: each value goes to its natural domain, and chained intermediates stay at the compute tier. ~300 ns/op. | Domains are mixed (decimal money + integers + symbolic fractions), the input is user/config-supplied, or you're chaining transcendentals and want the library to keep precision across the chain. |
| **Imperative / single-op** — `FixedPoint`, `DecimalFixed`, `FixedVector`, `FixedMatrix` | Direct `Copy` types; one call is one engine invocation, no expression tree. ~15 ns arithmetic. | You're in a hot loop and already know the domain — matrix code, ML inference, tight numeric kernels. |
| **Fused** — `imperative::fused` | Compute-tier accumulation (`softmax`, `silu`, `rms_norm_factor`, `euclidean_distance`). | ML hot paths, where intermediate materialisation would cost precision and time. |
| **Geometric / TQ1.9** | Manifolds and Lie groups; standalone ternary-quantised inference. | Geometric ML; weight-quantised inference. |

**Correctness is path-independent.** The imperative `DecimalFixed` surface and the
canonical `gmath()` surface compute through the same compute-tier engines and
round the same way — for a given input and precision they return the same,
correctly-rounded value. This is checked, not asserted: the decimal domain on
**both** paths — all 15 transcendentals plus `atan2` and `add`/`sub`/`mul`/`div`
— grades **0 LSB** against [mootable/decimal-scaled](https://github.com/mootable/decimal-scaled)'s
independent, adversarial mpmath corpus on every push. The harness, fixtures and
findings live on the [`validation/decimal-scaled`](https://github.com/nierto/gMath/tree/validation/decimal-scaled)
branch (`tests/decimal_contract_validation.rs`); all reference values are
generated by mpmath at high precision and committed as exact integer strings —
no floats anywhere in the comparison.

So choose by cost and ergonomics — router for mixed or chained work, imperative
for known-domain hot loops — and trust the number is the same either way.
